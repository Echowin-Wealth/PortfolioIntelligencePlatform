...
function toRow(
  ...
  const t =
    dateCell instanceof Date
      ? Date.UTC(
          dateCell.getUTCFullYear(),
          dateCell.getUTCMonth(),
          dateCell.getUTCDate(),
        )
  ...
  const v =
    typeof valCell === "number"
      ? valCell
      : parseFloat(String(valCell ?? "").replace(/,/g, ""));
  ...
  const onDrop = useCallback(
    async (files: File[]) => {
      ...
      try {
        ...
        if (rows.length === 0) {
          throw new Error(
            "No valid rows found (expected columns: Date, TRI Close)",
          );
        }
        setPreview(rows);
      } catch (err) {
    ...
    <div className="space-y-3">
      ...
      >
        ...
        <div className="mt-1 font-mono text-[11px] text-[var(--color-ink-soft)]">
          Columns:{" "}
          <span className="text-[var(--color-brand-700)]">Date, TRI Close</span>{" "}
          · a header row is auto-skipped
        </div>
      </div>
...
export function BenchmarkManager() {
  const [activeTab, setActiveTab] = useState<BenchmarkIndex>("N50");
  ...
  const loadData = useCallback(async (code: BenchmarkIndex) => {
    setLoading(true);
    ...
    setSummary({
      ...
      first:
        (earliestRes.data?.[0] as { date: string } | undefined)?.date ?? null,
  ...
  async function addRow() {
    ...
    const { error } = await supabase.from("benchmark_prices").upsert(
      {
  ...
  async function clearAll() {
    if (
      !confirm(`Delete all TRI rows for ${activeTab}? This cannot be undone.`)
    )
      return;
    const { error } = await supabase
      .from("benchmark_prices")
    ...
    <div className="space-y-8">
      <div>
        ...
        <p className="mt-1 text-[14px] text-[var(--color-ink-muted)]">
          TRI price series used by the alpha engine. Benchmark XIRR is computed
          from each fund's start date to the latest date in the series.
        </p>
      </div>

      <Tabs
        value={activeTab}
        onValueChange={(v) => setActiveTab(v as BenchmarkIndex)}
      >
        <TabsList>
          {TABS.map((t) => (
        ...
        {TABS.map((t) => (
          <TabsContent key={t} value={t} className="space-y-6">
            ...
            <Card className="overflow-hidden">
              ...
              <div className="p-6">
                <SeriesImport active={activeTab} onImport={handleImport} />
              </div>
            </Card>
          </TabsContent>
        ))}
